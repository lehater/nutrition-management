# Test Design Research — Nutrition Management
...
Does a design-complete project need a separate Test Design capability between Verification Design and coding, so a TDD coding agent does not decide what behavior, boundaries, failures or interactions must be tested?
...
If this split survives the experiment, Test Design is an independent semantic boundary. If Test Design merely repeats Verification Design or dictates test code structure, it should remain part of Verification Design instead.
...
Candidate Authority: `TEST-DESIGN`.

Candidate capability: `nutrition-management.test-design`.
...
Important dependency observation: most semantic Test Design can precede Implementation Design. Technology-specific integration-test design may depend on it. This suggests either:
1. Test Design depends on Component + Verification and Implementation Design depends on Test Design; or
2. semantic Test Design precedes Implementation Design, while implementation-specific verification realization is a downstream refinement.

For strict TDD readiness, option 1 is cleaner if Implementation Design is allowed to consume testability requirements.
